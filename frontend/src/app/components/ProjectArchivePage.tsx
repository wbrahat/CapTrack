...
const archivedProjects: ArchivedProject[] = [
  {
    ...
    // এখানে পরে তোমাদের আসল proposal link বসাবে
    reportUrl: "",

    // এখানে পরে তোমাদের আসল GitHub repository link বসাবে
    repositoryUrl: "",
  },
  {
...
export function ProjectArchivePage() {
  ...
  const semesterOptions = useMemo(
    () => [
      "All Semesters",
      ...Array.from(
        new Set(archivedProjects.map((project) => project.semester)),
      ),
    ],
    [],
  );
  ...
  const domainOptions = useMemo(
    () => [
      "All Domains",
      ...Array.from(new Set(archivedProjects.map((project) => project.domain))),
    ],
    [],
  );
  ...
  const filteredProjects = useMemo(() => {
    ...
    return archivedProjects.filter((project) => {
      const matchesSearch =
        ...
        project.group.toLowerCase().includes(normalizedSearch) ||
        project.semester.toLowerCase().includes(normalizedSearch);
      ...
      return matchesSearch && matchesSemester && matchesDomain;
    });
  }, [searchText, selectedSemester, selectedDomain]);

  const openExternalLink = (
    url: string,
    linkType: "report" | "repository",
  ) => {
    if (!url.trim()) {
      const message =
        linkType === "report"
          ? "Proposal/Report link is not available for this project."
          : "GitHub repository link is not available for this project.";
    ...
    try {
      ...
      window.alert("The provided link is invalid.");
    }
  };

  const handleAddArchiveEntry = () => {
    window.alert(
      "Add Archive Entry form has not been connected yet. This feature will be implemented in the next step.",
    );
  };
  ...
  return (
    <section className="min-h-screen bg-[#f3f6fa] px-8 py-8">
      <div className="mb-8 flex items-start justify-between gap-6">
        ...
        <button
          type="button"
          onClick={handleAddArchiveEntry}
          className="flex items-center gap-2 rounded-full bg-[#0f5f8f] px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-[#0b4f78]"
        >
          <Archive size={17} />
          Add Archive Entry
        </button>
                ...
                <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        openExternalLink(project.reportUrl, "report")
                      }
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 rounded-xl bg-[#0f5f8f] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#0b4f78]"
                    >
                      <Github size={16} />
                      {project.repository}
                    </button>
